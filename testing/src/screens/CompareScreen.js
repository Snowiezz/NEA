 ...
 *
 * The user selects 2–3 universities from a picker dropdown,
...
export default function CompareScreen({ navigation }) {
  const [selected, setSelected] = useState([]);    // array of university objects
  const [modalVisible, setModalVisible] = useState(false);

  // Add a university to comparison; enforce max 3
  const addUniversity = (uni) => {
    if (selected.find((u) => u.id === uni.id)) return; // already added
  ...
  // ── Column width depends on how many are selected ────────────────────────
      ...
      {selected.length < 2 ? (
        <View style={styles.placeholder}>
          ...
          <Text style={styles.placeholderBody}>
            Tap the Add button above to choose universities to compare.
          </Text>
        </View>
      ) : (
            ...
            <View>
              ...
              <View style={styles.tableRow}>
                <View style={styles.rowLabelCell}>
                  <Text style={styles.rowLabelHeader}>Criteria</Text>
                </View>
                {selected.map((uni) => (
                  <TouchableOpacity
                    key={uni.id}
