 ...
 *   - Save / unsave button (persisted)
 *   - Add / edit personal notes (persisted)
...
export default function UniversityDetailScreen({ route }) {
  ...
  const startEdit = () => { setDraftNote(noteText); setEditMode(true); };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <TouchableOpacity style={[styles.saveBtn, isSaved && styles.saveBtnActive]} onPress={toggleSaved}>
            ...
            <Text style={[styles.saveBtnText, isSaved && { color: '#fff' }]}>{isSaved ? 'Saved' : 'Save University'}</Text>
          </TouchableOpacity>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.metricsCard}>
              <View style={styles.ratingRow}><Text style={styles.ratingLabel}>Course Quality</Text><StarRating rating={uni.courseQualityRating} /></View>
              <View style={styles.ratingRow}><Text style={styles.ratingLabel}>Satisfaction</Text><StarRating rating={uni.studentSatisfaction} /></View>
              <View style={styles.ratingRow}><Text style={styles.ratingLabel}>Nightlife</Text><StarRating rating={uni.nightlifeRating} /></View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  banner: { backgroundColor: GREEN, padding: 24, paddingTop: 28 },
  ...
  rankBadge: { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 20, paddingVertical: 4, paddingHorizontal: 14 },
  rankBadgeText: { fontSize: 14, fontWeight: '700', color: '#fff' },
  saveBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, margin: 16, padding: 14, borderRadius: 12, borderWidth: 2, borderColor: GREEN, backgroundColor: '#fff' },
  ...
  metricsCard: { backgroundColor: '#fff', borderRadius: 14, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, elevation: 1 },
  metricRow: { flexDirection: 'row', alignItems: 'center', padding: 14, borderBottomWidth: 1, borderColor: '#f0f0f0', gap: 12 },
  metricIcon: { width: 36, height: 36, borderRadius: 10, backgroundColor: '#e8f5e9', justifyContent: 'center', alignItems: 'center' },
  metricLabel: { flex: 1, fontSize: 15, color: '#444' },
  metricValue: { fontSize: 15, fontWeight: '700', color: '#1a1a1a' },
  ratingRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 14, borderBottomWidth: 1, borderColor: '#f0f0f0' },
  ratingLabel: { fontSize: 15, color: '#444' },
  infoBox: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, backgroundColor: '#fff', borderRadius: 12, padding: 16 },
  ...
  noteInput: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 2, borderColor: GREEN, padding: 14, fontSize: 15, color: '#1a1a1a', minHeight: 120, textAlignVertical: 'top' },
  noteActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 10, marginTop: 10 },
  cancelBtn: { paddingVertical: 10, paddingHorizontal: 20, borderRadius: 10, backgroundColor: '#f0f0f0' },
  cancelBtnText: { fontSize: 15, color: '#555', fontWeight: '600' },
  saveNoteBtn: { paddingVertical: 10, paddingHorizontal: 20, borderRadius: 10, backgroundColor: GREEN },
  saveNoteBtnText: { fontSize: 15, color: '#fff', fontWeight: '600' },
  noteDisplayBox: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#e0e0e0', padding: 14, minHeight: 80 },
