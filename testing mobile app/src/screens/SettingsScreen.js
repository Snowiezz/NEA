...
export default function SettingsScreen({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      const load = async () => setPreferences(await loadPreferences());
      load();
    }, [])
  );
  ...
  const importanceLabel = (value) =>
    ({ 1: 'Not important', 2: 'Slightly', 3: 'Moderate', 4: 'Important', 5: 'Very important' })[value] ?? String(value);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          <View style={styles.aboutRow}><Text style={styles.aboutLabel}>App Version</Text><Text style={styles.aboutValue}>1.0.0</Text></View>
          ...
          <View style={styles.aboutRow}><Text style={styles.aboutLabel}>Purpose</Text><Text style={styles.aboutValue}>A-Level CS NEA Project</Text></View>
        </View>
...
const styles = StyleSheet.create({
  ...
  groupLabel: { fontSize: 12, fontWeight: '700', color: '#888', letterSpacing: 0.8, marginBottom: 8, marginTop: 16, marginLeft: 4 },
  card: { backgroundColor: '#fff', borderRadius: 14, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, elevation: 1 },
  prefRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 14, borderBottomWidth: 1, borderColor: '#f0f0f0' },
  prefLabel: { fontSize: 15, color: '#444' },
  prefValue: { fontSize: 15, fontWeight: '600', color: '#1a1a1a' },
  noPrefBox: { padding: 24, alignItems: 'center', gap: 8 },
  noPrefText: { fontSize: 15, color: '#aaa' },
  settingsBtn: { flexDirection: 'row', alignItems: 'center', padding: 16, gap: 14, borderBottomWidth: 1, borderColor: '#f0f0f0' },
  settingsBtnIcon: { width: 44, height: 44, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  settingsBtnText: { flex: 1 },
  settingsBtnLabel: { fontSize: 15, fontWeight: '600' },
  settingsBtnDesc: { fontSize: 13, color: '#888', marginTop: 2 },
  aboutRow: { flexDirection: 'row', justifyContent: 'space-between', padding: 14, borderBottomWidth: 1, borderColor: '#f0f0f0' },
