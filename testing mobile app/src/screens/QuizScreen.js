...
const QUESTIONS = [
  ...
  },
  {
    ...
    options: [
      ...
      { label: 'Somewhere else', lat: 52.4862, lon: -1.8904 },
    ],
  },
];
...
export default function QuizScreen({ navigation }) {
  ...
  const setSlider = (value) => setAnswers((prev) => ({ ...prev, [question.id]: value }));

  const setNumber = (text) => {
    const num = parseInt(text, 10);
    if (!isNaN(num)) setAnswers((prev) => ({ ...prev, [question.id]: num }));
  };
        ...
        <View style={styles.sliderButtons}>
          {[1, 2, 3, 4, 5].map((n) => (
            <TouchableOpacity
              ...
              onPress={() => setSlider(n)}
            >
              <Text style={[styles.sliderBtnText, value === n && styles.sliderBtnTextActive]}>{n}</Text>
            </TouchableOpacity>
          ))}
        </View>
    ...
    <View style={styles.choiceContainer}>
      {question.options.map((option) => {
        ...
        return (
          ...
          >
            <Text style={[styles.choiceBtnText, selected && styles.choiceBtnTextActive]}>{option.label}</Text>
...
const styles = StyleSheet.create({
  ...
  progressBar: { height: 6, backgroundColor: '#e0e0e0' },
  progressFill: { height: 6, backgroundColor: GREEN, borderRadius: 3 },
  progressText: { textAlign: 'center', fontSize: 13, color: '#888', marginTop: 8 },
  scrollContent: { padding: 24, paddingBottom: 16 },
  questionTitle: { fontSize: 22, fontWeight: '700', color: '#1a1a1a', marginBottom: 8, lineHeight: 30 },
  questionSubtitle: { fontSize: 15, color: '#666', marginBottom: 32 },
  answerSection: { marginTop: 8 },
  sliderContainer: { alignItems: 'center' },
  sliderValue: { fontSize: 28, fontWeight: 'bold', color: GREEN, marginBottom: 20 },
  sliderButtons: { flexDirection: 'row', gap: 12 },
  sliderBtn: {
    width: 56, height: 56, borderRadius: 28, borderWidth: 2,
    borderColor: '#ccc', backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center',
  },
  sliderBtnActive: { borderColor: GREEN, backgroundColor: GREEN },
  sliderBtnText: { fontSize: 20, fontWeight: '600', color: '#555' },
  sliderBtnTextActive: { color: '#fff' },
  sliderLabels: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', marginTop: 12, paddingHorizontal: 4 },
  sliderLabelText: { fontSize: 12, color: '#999' },
  numberContainer: { alignItems: 'center' },
  numberInputRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  numberInput: {
    width: 140, height: 64, borderWidth: 2, borderColor: GREEN, borderRadius: 12,
    textAlign: 'center', fontSize: 28, fontWeight: 'bold', color: '#1a1a1a', backgroundColor: '#fff',
  },
  unitLabel: { fontSize: 18, color: '#555', fontWeight: '600' },
  choiceContainer: { gap: 10 },
  choiceBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    backgroundColor: '#fff', borderRadius: 12, borderWidth: 2, borderColor: '#e0e0e0', padding: 16,
  },
  choiceBtnActive: { borderColor: GREEN, backgroundColor: '#e8f5e9' },
  choiceBtnText: { fontSize: 16, color: '#333', fontWeight: '500' },
  choiceBtnTextActive: { color: GREEN, fontWeight: '700' },
  navRow: {
    flexDirection: 'row', justifyContent: 'space-between', padding: 20,
    borderTopWidth: 1, borderColor: '#eee', backgroundColor: '#fff',
  },
  navBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 14, paddingHorizontal: 24, borderRadius: 12 },
  backBtn: { backgroundColor: '#f0f0f0' },
  nextBtn: { backgroundColor: GREEN },
  navBtnDisabled: { opacity: 0.5 },
  navBtnText: { fontSize: 16, fontWeight: '600', color: GREEN },
