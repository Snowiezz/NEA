...
export default function SavedScreen({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      const load = async () => {
        ...
        const unis = UNIVERSITIES.filter((u) => ids.includes(u.id));
        setSavedUnis(unis);
  ...
  const handleRemove = (uni) => {
    Alert.alert(
      'Remove from Saved',
      `Remove ${uni.name} from your saved list?`,
      [
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyBody}>
            Open any university from the Recommendations screen and tap Save.
          </Text>
          <TouchableOpacity
