 ...
 *
 * Uses the rankUniversities() function from the recommendation engine to
 * compute scores, then renders each university as a card with:
 *   - rank position
 *   - match score (0–100)
...
export default function RecommendationsScreen({ navigation }) {
  ...
  // Reload every time the screen gains focus (e.g. after taking the quiz)
  useFocusEffect(
    useCallback(() => {
      const load = async () => {
        setLoading(true);
        ...
        const results = rankUniversities(prefs);
        setRanked(results);
        setLoading(false);
      };
      load();
